...
// Assets from: https://github.com/volcanioo/Human-Body-Rendering-HTML
...
const BODY_PARTS_DATA = {
    ...
    'right-shoulder': {
        width: 109.532, height: 46.594, viewBox: '0 0 109.532 46.594',
        style: { marginLeft: '13.5px', top: '69px' },
    ...
    'right-arm': {
        width: 156.344, height: 119.25, viewBox: '0 0 156.344 119.25',
        style: { marginLeft: '38px', top: '112px', zIndex: '10001' },
    ...
    'left-leg': {
        ...
        style: { marginLeft: '-46.5px', top: '205px', zIndex: '9999' },
    ...
    'right-leg': {
        ...
        style: { marginLeft: '1.5px', top: '205px', zIndex: '9999' },
    ...
    'right-hand': {
        width: 90, height: 38.938, viewBox: '0 0 90 38.938',
        style: { marginLeft: '66.5px', top: '224px', zIndex: '10000' },
    ...
    'left-foot': {
        ...
        style: { marginLeft: '-35.5px', top: '455px' },
    ...
    'right-foot': {
        width: 90, height: 38.938, viewBox: '0 0 90 38.938',
        style: { marginLeft: '5.5px', top: '455px' },
...
// Map Front Parts to Back Parts for the Logic
// Front Part ID -> Back Part Logical Name
const BACK_VIEW_MAPPING = {
    'head': 'neck',
    'chest': 'upper_back',
    'stomach': 'lower_back',
    'left-shoulder': 'right_shoulder_back', // Flipped: Left Visual -> Right Anatomical
    'right-shoulder': 'left_shoulder_back',
    'left-arm': 'right_arm_back',
    'right-arm': 'left_arm_back',
    'left-hand': 'right_hand_back',
    'right-hand': 'left_hand_back',
    'left-leg': 'right_hamstrings',
    'right-leg': 'left_hamstrings',
    'left-foot': 'right_foot_back',
    'right-foot': 'left_foot_back'
};
...
const DISPLAY_NAMES = {
    ...
    'left-arm': 'Left Arm (Biceps/Forearm)',
    'right-arm': 'Right Arm (Biceps/Forearm)',
    'left_arm_back': 'Left Arm (Triceps)',
    'right_arm_back': 'Right Arm (Triceps)',
    'left-hand': 'Left Hand',
    'right-hand': 'Right Hand',
    'left_hand_back': 'Left Hand',
    'right_hand_back': 'Right Hand',
    'left-leg': 'Left Leg (Quads)',
    'right-leg': 'Right Leg (Quads)',
    'left_hamstrings': 'Left Leg (Hamstrings)',
    'right_hamstrings': 'Right Leg (Hamstrings)',
    'left-foot': 'Left Foot',
    'right-foot': 'Right Foot',
    'left_foot_back': 'Left Foot',
    'right_foot_back': 'Right Foot'
};

// Advice Dictionary
const ADVICE = {
    "head": "For headaches: Rest in a dark room, stay hydrated, and try gentle neck stretches.",
    "neck": "For neck pain: Gentle neck rolls, heat therapy, and check your posture/pillow.",
    "chest": "For chest soreness: Gentle stretching (doorway stretch), rest, and light movement.",
    "upper_back": "For upper back pain: Thoracic extension stretches, foam rolling, and posture correction.",
    "stomach": "For abdominal soreness: Gentle cobra stretch, hydration, and magnesium.",
    "lower_back": "For lower back pain: Cat-cow stretches, child's pose, and avoid heavy lifting.",
    "shoulders": "For shoulder pain: Pendulum swings, cross-body stretch, and rotator cuff strengthening.",
    "arms": "For arm pain: Gentle stretching, massage, and rest from repetitive motions.",
    "hands": "For hand pain: Wrist flexor/extensor stretches, stress ball squeezing, and warm water soak.",
    "legs": "For leg soreness: Foam rolling, elevate legs, and dynamic stretching.",
    "hamstrings": "For hamstring tightness: Standing forward bend, seated hamstring stretch, and warmth.",
    "feet": "For foot pain: Roll foot over a tennis ball, calf stretches, and supportive footwear."
};

export function initBodyMap(frontId, backId) {
    if (frontId) renderView(frontId, false);
    if (backId) renderView(backId, true);
}

function renderView(containerId, isBack) {
    ...
    if (isBack) wrapper.classList.add('back-view');

    Object.keys(BODY_PARTS_DATA).forEach(key => {
        const part = BODY_PARTS_DATA[key];

        // Create SVG element
        const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
        svg.setAttribute("viewBox", part.viewBox);
        svg.setAttribute("width", part.width);
        svg.setAttribute("height", part.height);
        svg.setAttribute("id", key + (isBack ? '_back' : '')); // Unique IDs if needed

        // Apply Styles
        Object.assign(svg.style, part.style);
        ...
        svg.addEventListener('click', (e) => {
            e.stopPropagation();
            handlePartClick(key, isBack);

            // Visual Selection
            const allSvgs = document.querySelectorAll('.human-body svg');
            allSvgs.forEach(s => s.classList.remove('selected'));
...
function handlePartClick(originalKey, isBack) {
    let logicalPartId = originalKey;
    if (isBack) {
        logicalPartId = BACK_VIEW_MAPPING[originalKey] || originalKey;
    }

    const genericKey = getGenericAdviceKey(logicalPartId);
    const advice = ADVICE[genericKey] || "Gentle stretching and rest are generally recommended. Consult a professional if pain persists.";

    const event = new CustomEvent('bodyPartSelected', {
        detail: {
            partId: logicalPartId,
            name: name,
            advice: advice
        }
    });
...
function getGenericAdviceKey(partId) {
    if (partId.includes('shoulder')) return 'shoulders';
    if (partId.includes('arm')) return 'arms';
    if (partId.includes('hand')) return 'hands';
    if (partId.includes('leg') || partId.includes('quads')) return 'legs';
    if (partId.includes('hamstring')) return 'hamstrings';
    if (partId.includes('foot') || partId.includes('feet')) return 'feet';
    return partId; // head, neck, chest, etc.
}
